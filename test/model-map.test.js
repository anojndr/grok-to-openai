...
test("resolveModel detects mode aliases embedded in model names", () => {
  ...
  assert.equal(resolveModel("grok-4.3-heavy").grokModeId, "heavy");
  assert.equal(resolveModel("grok heavy").grokModeId, "heavy");
  assert.equal(resolveModel("grok-4.3-fast").grokModeId, "fast");
  assert.equal(resolveModel("grok-4.3-expert").grokModeId, "expert");
  assert.equal(resolveModel("grok-4.3-heavy").grokModeId, "heavy");
  assert.equal(resolveModel("grok-4.3-auto").grokModeId, "auto");
  assert.equal(resolveModel("grok-4.3").grokModeId, "auto");
  assert.equal(
...
test("listModels exposes Grok 4.3 models", () => {
  assert.ok(listModels().some((model) => model.id === "grok-4.3-heavy"));
  assert.ok(listModels().some((model) => model.id === "grok-4.3-beta"));
  assert.ok(listModels().some((model) => model.id === "grok-4.3-fast"));
  assert.ok(listModels().some((model) => model.id === "grok-4.3-expert"));
});
